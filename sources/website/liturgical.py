...
OUTPUT_PATH = ROOT / "raw" / "liturgical" / f"{datetime.now().year}.jsonl"
...
class UniversalisCalendarParser(HTMLParser):
    ...
    def handle_data(self, data):
        if not self.in_calendar or not self.in_row:
            return
        if self.in_month_heading:
            value = clean_text(data)
            try:
                self.current_month = datetime.strptime(value, "%B").month
            except ValueError:
                pass
        elif self.cell_index == 0 and self.current_date is None and self.current_month:
            match = re.search(r"\b(\d{1,2})\b", clean_text(data))
            if match:
                self.current_date = date(self.year, self.current_month, int(match.group(1)))
...
def season_boundaries(rows, year):
...
def build_records(rows, year):
    boundaries = season_boundaries(rows, year)
    records = []
    for row in rows:
        if not row["segments"]:
            continue
...
def build_calendar_records(calendar_html, year=YEAR):
    parser = UniversalisCalendarParser(year)
    parser.feed(calendar_html)
    records = build_records(parser.rows, year)
    validate(records, year)
    return records


def calendar_url(year):
    filename = "calendar.htm" if year == datetime.now().year else f"calendar-{year}.htm"
    return f"https://universalis.com/{CALENDAR_REGION}/1000/{filename}"


def fetch_calendar_html(year=YEAR):
...
def main():
    records = build_calendar_records(fetch_calendar_html(YEAR), YEAR)
    write_records(records, ROOT / "raw" / "liturgical" / f"{YEAR}.jsonl")
...
def fetch(year=YEAR):
    return fetch_calendar_html(year)


def normalise(calendar_html, year=YEAR):
    return build_calendar_records(calendar_html, year)
